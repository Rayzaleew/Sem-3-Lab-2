...
Time operator+(const Time& time_a, const Time& time_b) {	//Перегрузка оператора вывода
	Time time3(time_a.hour + time_b.hour, time_a.minute + time_b.minute, time_a.second + time_b.second, time_a.msecond + time_b.msecond);
...
Time operator-(Time& time_a, Time& time_b) {	//Перегрузка оператора вычитания
	...
	long long t1_all_in_mc = (time_a.hour * (60 * 60 * 1000)) + (time_a.minute * (60 * 1000)) + (time_a.second * 1000) + time_a.msecond;
	long long t2_all_in_mc = (time_b.hour * (60 * 60 * 1000)) + (time_b.minute * (60 * 1000)) + (time_b.second * 1000) + time_b.msecond;
	long long t3_all_in_mc = (t1_all_in_mc > t2_all_in_mc) ? (t1_all_in_mc - t2_all_in_mc) : (t2_all_in_mc - t1_all_in_mc);
...
int main() {
	...
	//Задание к работе: Написать программу на языке С++, которая складывает и вычитает время (часы, минуты, секунды и миллисекунды) с использованием классов. Использовать конструкторы по умолчанию и параметризованные. Перегрузить операторы сложения и вычитания, а также ввода и вывода на 3 потока (строковый, консоль, файловый).
	...
	int choise;
	cout << "Выберите тип ввода и вывода данных: " << endl << "1 - ввод с консоли и вывод в консоль" << endl << "2 - ввод с файла и вывод в файл" << endl << "3 - поток вывода" << endl << ": ";
	do {
		cin >> choise;
		cout << endl;
		if (choise < 1 || choise > 3) cout << "Вы неверно ввели номер варианта типа ввода и вывода данных. Попробуйте еще раз: ";
	...
	case 2: {
		ifstream file1("in.txt");	//Инициализация file1 в качестве объекта потока ввода
		ofstream file2("out.txt");	//Инициализация file2 в качестве объекта потока вывода
	...
	case 3: {
		...
		oss << "Разница во времени между time_a и time_b: " << time_a - time_b << endl;
		//Вывод данных, содержащихся в объекте строкового потока вывода, в виде строки в консоль
		cout << oss.str();
		break;
	}
	}
